/**
 * Regression test for AISDLC-257 — orchestrator fixture leak prevention.
 *
 * Mirrors `pipeline-cli/src/__test-helpers/git-env.test.ts` (AISDLC-253).
 *
 * The leak: when worktree-pool.integration.test.ts (and other tests) ran with
 * the parent shell exporting GIT_DIR=/path/to/host/worktree/.git (e.g.
 * inherited from a husky pre-push hook), git commands that used `cleanGitEnv()`
 * (which inherits `process.env` minus GIT_DIR) could still have `git config
 * user.email` write into unexpected locations. More critically, any test that
 * did NOT pass env at all would have ALL git commands follow the polluted
 * GIT_DIR, writing into the HOST worktree's branch.
 *
...
describe('makeGitEnv() — AISDLC-257 orchestrator fixture-leak prevention', () => {
  ...
  // path. This is the regression that would have caught AISDLC-257 at
  // CI-time before the operator hit it in production.
  ...
  describe('end-to-end leak reproduction', () => {
    ...
    it('UN-hardened path: confirms the leak DOES reproduce when GIT_ENV is omitted (canary)', () => {
      ...
      // We expect this to commit into hostDir's repo, not fixtureDir.
      try {
